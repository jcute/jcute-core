...
public interface PathMatcher{
	 ...
	 * <p>
	 * This method is supposed to find out which part of the path is matched dynamically through an actual pattern, that is, it strips off a statically defined leading path from the given full path, returning only the actually pattern-matched part of
	 * the path.
	 * <p>
	 ...
	 * <p>
	 * A simple implementation may return the given full path as-is in case of an actual pattern, and the empty String in case of the pattern not containing any dynamic parts (i.e. the {@code pattern} parameter being a static path that wouldn't
	 * qualify as an actual {@link #isPattern pattern}). A sophisticated implementation will differentiate between the static parts and the dynamic parts of the given path pattern.
	 * 
	 ...
	 * <p>
	 * The full algorithm used depends on the underlying implementation, but generally, the returned {@code Comparator} before generic patterns.
	 * 
